...
class View_Service():
    ...
    def filter_on_date(self, game_information_list):
        for passnum in range(len(game_information_list)-1,0,-1):
            for i in range(passnum):
                date1 = datetime.datetime.strptime(game_information_list[i]['date'], '%Y-%m-%d')
                date2 = datetime.datetime.strptime(game_information_list[i+1]['date'], '%Y-%m-%d')
                if date1>date2:
    ...
    def print_game_information(self, filtered_games):
        ...
        print('game:', filtered_games['home'], filtered_games['away'], '|compare|' , filtered_games['compared_home'], filtered_games['compared_away'])
        print('minimal betting odd:', filtered_games['minimal_betting_odd'])
        print('probability to win this game:', filtered_games['winning_side_probability'], 'on', filtered_games['winning_side'])
        
        if filtered_games['best_bookmaker'] != '':
            print('best one is ' + filtered_games + ' with quote ' + str(filtered_games))
        else:
